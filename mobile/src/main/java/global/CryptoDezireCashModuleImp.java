...
import global.cryptodezirecash.DefaultCoinSelector;
...
public class CryptoDezireCashModuleImp implements CryptoDezireCashModule {

    private static final Logger logger = LoggerFactory.getLogger(CryptoDezireCashModuleImp.class);
    ...
    public CryptoDezireCashModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStoreDao contactsStore,RateDbDao rateDb,BackupHelper backupHelper) {
...
//    }
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in cryptodezirecash core)
...
//                    }
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<CryptoDezireCashRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(CryptoDezireCashRate cryptodezirecashRate){
        rateDb.insertOrUpdateIfExist(cryptodezirecashRate);
    }
