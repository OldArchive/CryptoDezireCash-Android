package cryptodezirecash.org.cryptodezirecashwallet.service;
...
import org.cryptodezirecashj.core.FilteredBlock;
...
import org.cryptodezirecashj.core.listeners.AbstractPeerDataEventListener;
...
import cryptodezirecash.org.cryptodezirecashwallet.module.store.SnappyBlockchainStore;
import cryptodezirecash.org.cryptodezirecashwallet.rate.CoinMarketCapApiClient;
import cryptodezirecash.org.cryptodezirecashwallet.rate.RequestCryptoDezireCashRateException;
...
import static cryptodezirecash.org.cryptodezirecashwallet.module.CryptoDezireCashContext.CONTEXT;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
...
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.DATA_TRANSACTION_HASH;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE;
...
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_TYPE;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static cryptodezirecash.org.cryptodezirecashwallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by akshaynexus on 6/12/17.
 */

public class CryptoDezireCashWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(CryptoDezireCashWalletService.class);

    private CryptoDezireCashApplication cryptodezirecashApplication;
    private CryptoDezireCashModuleImp module;
...
//    NotificationManager mNotificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    public class CryptoDezireCashBinder extends Binder {
        public CryptoDezireCashWalletService getService() {
            return CryptoDezireCashWalletService.this;
        }
    }
    ...
    public IBinder onBind(Intent intent) {
        return new CryptoDezireCashBinder();
    }
    ...
    private final PeerDataEventListener blockchainDownloadListener = new AbstractPeerDataEventListener() {
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (CryptoDezireCashContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    cryptodezirecashApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
                    broadcastBlockchainState(true);
                }
            }catch (Exception e){
                e.printStackTrace();
                CrashReporter.saveBackgroundTrace(e,cryptodezirecashApplication.getPackageInfo());
            }
        }
    };
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.cryptodezirecashj.core.Context.propagate(CryptoDezireCashContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.cryptodezirecashj.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), CryptoDezireCashWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(CryptoDezireCashWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                        ...
                        mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(CryptoDezireCashWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("CryptoDezireCash service started");
            // Android stuff
            ...
            module = (CryptoDezireCashModuleImp) cryptodezirecashApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to pivtrum node
            /*pivtrumPeergroup = new PivtrumPeergroup(cryptodezirecashApplication.getNetworkConf());
            ...
            String filename = CryptoDezireCashContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(CryptoDezireCashContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("CryptoDezireCash service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void requestRateCoin(){
        final AppConf appConf = cryptodezirecashApplication.getAppConf();
        CryptoDezireCashRate cryptodezirecashRate = module.getRate(appConf.getSelectedRateCoin());
        if (cryptodezirecashRate == null || cryptodezirecashRate.getTimestamp() + CryptoDezireCashContext.RATE_UPDATE_TIME < System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.CryptoDezireCashMarket cryptodezirecashMarket = c.getCryptoDezireCashPxrice();
                        CryptoDezireCashRate cryptodezirecashRate = new CryptoDezireCashRate("USD",cryptodezirecashMarket.priceUsd,System.currentTimeMillis());
                        module.saveRate(cryptodezirecashRate);
                        final CryptoDezireCashRate cryptodezirecashBtcRate = new CryptoDezireCashRate("BTC",cryptodezirecashMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(cryptodezirecashBtcRate);

                        // Get the rest of the rates:
                        List<CryptoDezireCashRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<CryptoDezireCashRate>() {
                            @Override
                            public CryptoDezireCashRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(cryptodezirecashBtcRate.getRate());
                                return new CryptoDezireCashRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (CryptoDezireCashRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestCryptoDezireCashRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
