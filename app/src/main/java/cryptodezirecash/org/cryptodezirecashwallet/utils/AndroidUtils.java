...
public class AndroidUtils {
    ...
    public static void shareText(final Context context, final CharSequence subject, final CharSequence text, final ArrayList<Uri> attachments) {
        ...
        intent.putExtra(Intent.EXTRA_EMAIL, new String[]{CryptoDezireCashContext.REPORT_EMAIL});
        if (subject != null)
...
//		str.add(text);
        ...
        try {
            ...
            if (!(context instanceof Activity)){
                CryptoDezireCashApplication.getInstance().getAppConf().saveShowReportScreenOnStart(true);
