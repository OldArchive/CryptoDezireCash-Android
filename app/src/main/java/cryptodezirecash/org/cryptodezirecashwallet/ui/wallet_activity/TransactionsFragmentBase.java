...
import cryptodezirecash.org.cryptodezirecashwallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static cryptodezirecash.org.cryptodezirecashwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static cryptodezirecash.org.cryptodezirecashwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static cryptodezirecash.org.cryptodezirecashwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = cryptodezirecashModule.listTx();
        ...
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (cryptodezirecashRate!=null) {
                    localCurrency = cryptodezirecashApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * cryptodezirecashRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                ...
                holder.title.setText(getAddressOrContact(cryptodezirecashModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(cryptodezirecashModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(cryptodezirecashModule.getConf().getNetworkParams()).toBase58());
                }*/
