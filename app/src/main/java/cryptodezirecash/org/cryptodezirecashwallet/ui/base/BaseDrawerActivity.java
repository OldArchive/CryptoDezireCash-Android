...
import cryptodezirecash.org.cryptodezirecashwallet.ui.contacts_activity.ContactsActivity;
import cryptodezirecash.org.cryptodezirecashwallet.ui.donate.DonateActivity;
import cryptodezirecash.org.cryptodezirecashwallet.ui.settings_activity.SettingsActivity;
import cryptodezirecash.org.cryptodezirecashwallet.ui.wallet_activity.WalletActivity;

import static cryptodezirecash.org.cryptodezirecashwallet.module.CryptoDezireCashContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends CryptoDezireCashActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = cryptodezirecashApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = cryptodezirecashModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (cryptodezirecashModule.getChainHeight()+10>peerHeight) {
            ...
            if (cryptodezirecashModule.isAnyPeerConnected()) {
                long peerHeight = cryptodezirecashModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (cryptodezirecashModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
