package cryptodezirecash.org.cryptodezirecashwallet.ui.pincode_activity;
...
import cryptodezirecash.org.cryptodezirecashwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import cryptodezirecash.org.cryptodezirecashwallet.ui.base.BaseActivity;
import cryptodezirecash.org.cryptodezirecashwallet.ui.settings_pincode_activity.KeyboardFragment;
import cryptodezirecash.org.cryptodezirecashwallet.ui.start_activity.StartActivity;

import static cryptodezirecash.org.cryptodezirecashwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (cryptodezirecashApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (cryptodezirecashApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            cryptodezirecashApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        cryptodezirecashApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(cryptodezirecashApplication.getAppConf().getPincode().equals(pincode)){
