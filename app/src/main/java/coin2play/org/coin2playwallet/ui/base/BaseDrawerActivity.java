...
import coin2play.org.coin2playwallet.ui.contacts_activity.ContactsActivity;
import coin2play.org.coin2playwallet.ui.donate.DonateActivity;
import coin2play.org.coin2playwallet.ui.settings_activity.SettingsActivity;
import coin2play.org.coin2playwallet.ui.wallet_activity.WalletActivity;

import static coin2play.org.coin2playwallet.module.Coin2PlayContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends Coin2PlayActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = coin2playApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = coin2playModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (coin2playModule.getChainHeight()+10>peerHeight) {
            ...
            if (coin2playModule.isAnyPeerConnected()) {
                long peerHeight = coin2playModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (coin2playModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = coin2playModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (coin2playModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
