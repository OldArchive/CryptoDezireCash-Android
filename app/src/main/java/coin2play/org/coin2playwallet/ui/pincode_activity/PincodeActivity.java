package coin2play.org.coin2playwallet.ui.pincode_activity;
...
import coin2play.org.coin2playwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import coin2play.org.coin2playwallet.ui.base.BaseActivity;
import coin2play.org.coin2playwallet.ui.settings_pincode_activity.KeyboardFragment;
import coin2play.org.coin2playwallet.ui.start_activity.StartActivity;

import static coin2play.org.coin2playwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (coin2playApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (coin2playApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            coin2playApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        coin2playApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(coin2playApplication.getAppConf().getPincode().equals(pincode)){
