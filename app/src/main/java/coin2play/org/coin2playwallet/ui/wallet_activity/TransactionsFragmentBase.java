...
import coin2play.org.coin2playwallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static coin2play.org.coin2playwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static coin2play.org.coin2playwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static coin2play.org.coin2playwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = coin2playModule.listTx();
        ...
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (coin2playRate!=null) {
                    localCurrency = coin2playApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * coin2playRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                ...
                holder.title.setText(getAddressOrContact(coin2playModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(coin2playModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(coin2playModule.getConf().getNetworkParams()).toBase58());
                }*/
