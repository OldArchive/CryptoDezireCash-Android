package coin2play.org.coin2playwallet.rate.db;
...
import coin2play.org.coin2playwallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<Coin2PlayRate> implements RateDbDao<Coin2PlayRate> {
    ...
    @Override
    protected ContentValues buildContent(Coin2PlayRate obj) {
    ...
    @Override
    protected Coin2PlayRate buildFrom(Cursor cursor) {
        ...
        return new Coin2PlayRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(Coin2PlayRate coin2playRate) {
        if (getRate(coin2playRate.getCode())==null){
            insert(coin2playRate);
        }else {
            updateByKey(KEY_COIN,coin2playRate.getCode(),coin2playRate);
        }
    }
}
